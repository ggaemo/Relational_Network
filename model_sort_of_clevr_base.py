...
class RelationalNetwork():
    ...
    def __init__(self, inputs, qst_color_vocab_size, qst_type_vocab_size, ans_vocab_size,
        ...
        def build_coord_tensor(batch_size, height):
            ...
            # coord = tf.linspace(-1.0, 1.0, height)
        ...
        with tf.variable_scope('image_embedding'):
            ...
            encoded_img_coord = tf.concat([encoded_img, tf.zeros_like(coord_tensor)],
                                          axis=3)
        ...
        with tf.variable_scope('img_qst_concat'):
            ...
            encoded_qst_expand = tf.reshape(encoded_qst,
                                            [batch_size, 1, 1, qst_type_vocab_size + qst_color_vocab_size])
        ...
        # with tf.variable_scope('img_qst_concat'):
        #
        #     encoded_img_flatten = tf.layers.flatten(encoded_img_coord)
        #     encoded_img_qst = tf.concat([encoded_img_flatten, encoded_qst], axis=1)
        ...
        with tf.variable_scope('g_theta'):
            ...
            #                                                   keep_dims=True)
            tf.add_to_collection('g_theta', output)

            # pair_output_sum = output

            pair_output_sum = tf.reduce_sum(output, (1, 2))
